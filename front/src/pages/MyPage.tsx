...
const MyPage=()=> {
    ...
    const { data: scheduleData, error: scheduleError } = useQuery(['scehduler'], () => api.getData<MySchedule>(`/scheduler`));
    useEffect(() => {
                            ...
                            <Slider {...settings}>
                                {Object.entries(schedule).map(([s,info])=>(
                                  <div key={s} className="flex justify-center p-2 text-xs sm:text-base" onClick={navigateAllSchedule}>
                                    ...
                                    <img
                                        src={info.imagePath}
                                        className="mb-2 object-cover h-40"
                                        onClick={() => handleSchedulerClick(info.schedulerId)}
                                        alt="Scheduler Image"
                                      />
                                    </figure>
                                  </div>
