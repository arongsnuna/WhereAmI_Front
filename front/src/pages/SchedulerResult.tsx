      ...
      <div className="bg-white p-8 m-5 rounded-lg shadow-lg border border-gray-100">
        <div className="flex justify-center">
          <img
            src={imagePath}
            alt={imageName}
            className="w-1/2 h-auto mb-4 rounded items-center"
          />
        </div>
        <h2
        ...
        <br />
        {description && (
          <p style={{ fontFamily: "GmarketSansMedium" }}>설명: {description}</p>
        )}
        <br />
        {recommendPlace && (
          <p style={{ fontFamily: "GmarketSansMedium" }}>
            주변 장소: {recommendPlace}
          </p>
        )}
        <br />
        <div className="flex justify-center">
...
const SchedulerResult = () => {
  ...
  const [oneScheduleItems, setOneScheduleItems] = useState<{
    [date: string]: ScheduleItem[];
  }>({});
  const { schedulerId } = useParams();
  const { data: oneScheduleData, isLoading: isOneScheduleDataLoading } =
    useQuery(["scehduler"], () =>
  ...
  return (
    <div>
      <div className="flex">
        ...
        <div
          className="w-3/5 text-center pt-5 text-5xl"
          style={{ fontFamily: "GangwonEduPowerExtraBoldA" }}
        ...
        <div className="w-1/5 flex justify-end pt-5 pr-5">
          <button
            className="bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-3 rounded my-auto text-xs sm:text-base"
            onClick={handleLogin}
          >
            {buttonText}
          </button>
        ...
        {oneSchedule && (
          <div>
            <div className="w-full flex">
              ...
              <h1
                className="w-full m-3 p-2 items-center text-center text-bold border border-gray-200 rounded"
                style={{ fontFamily: "GangwonEduPowerExtraBoldA" }}
              >
                {oneSchedule.title}
              </h1>
            ...
            <Slider {...settings}>
              {oneScheduleItems &&
                Object.keys(oneScheduleItems).map((date) => (
                          ...
                          <div className="flex justify-center">
                            <figure key={index}>
                              {item.transportation == "" ? (
                                ...
                                <p
                                  className="p-1 text-center justify-center m-5"
                                  style={{ fontFamily: "GmarketSansMedium" }}
                                >
                                  -- 이동 방법 --
                                  <br />
                              ...
                              <img
                                onClick={() =>
                                  openImageModal(item.imagePath, item)
                                }
                                src={item.imagePath}
                                className="p-1"
                              />
