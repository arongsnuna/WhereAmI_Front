...
import { LocalLandmark } from '../types/types';
...
const Search = () => {
    ...
    const [landmark, setLandmark] = useState<LocalLandmark | null>(null);
    const [nearByLandmarks, setNearByLandmarks] = useState<LocalLandmark[] | null>(null);

    const handleImageUpload = async (file: File) => {
        try {
            const formData = new FormData();
            formData.append('file', file);
            const response = (await postData('/image', formData, config) as unknown) as ImageResponse;

            if (!response.landmark.address) {
                response.landmark.address = "Unknown Address";
            }
            
            if (!response.landmark.imagePath) {
                response.landmark.imagePath = "path_to_default_image.png"; 
            }
    ...
    return (
        ...
        {landmark && userState.user && <LandmarkResult landmark={landmark} nearByLandmarks={nearByLandmarks || []} userId={userState.user}/>}
        </div>
    )
}
